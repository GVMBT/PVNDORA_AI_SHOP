...
## Рекомендуемый подход: Admin Bot 🤖

**Отдельный бот для управления рассылками** — `@pvndora_admin_bot`

### Преимущества

| Аспект | Плюс |
|--------|------|
| **Изоляция** | Админские функции отделены от пользовательских |
| **Безопасность** | Доступ только авторизованным админам |
| **Удобство** | Нативный Telegram интерфейс вместо веб-панели |
| **Гибкость** | Можно рассылать в разные боты (PVNDORA/discount) |

### Архитектура Admin Bot
...
│                                                                 │
│  /broadcast                                                     │
│  ├── 🤖 Выберите бота: [PVNDORA] [Discount]                    │
│  ├── 🎯 Аудитория: [Все] [VIP] [Активные] [Неактивные]         │
│  ├── 🌐 Языки: [RU] [EN] [DE] [Все]                            │
│  ├── 📝 Контент: (отправьте текст/фото/видео)                  │
│  ├── 🔘 Кнопки: (опционально)                                   │
│  └── ✅ Отправить / ⏰ Запланировать                            │
│                                                                 │
...
### Пример флоу создания рассылки

```
Админ: /broadcast

Бот:   🤖 Выберите целевого бота:
       [PVNDORA] [Discount]

Админ: [PVNDORA]

Бот:   🎯 Выберите аудиторию:
       [Все] [VIP-партнёры] [Активные 7д] [Неактивные 7д+]
       [Покупатели] [Без покупок]

Админ: [Активные 7д]

Бот:   🌐 Выберите языки (можно несколько):
       [🇷🇺 RU] [🇬🇧 EN] [🇩🇪 DE] [🇫🇷 FR] [🇪🇸 ES]
       [Все языки]

Админ: [🇷🇺 RU] [🇬🇧 EN]

Бот:   📝 Теперь отправьте контент рассылки.
       ...
       💡 Подсказка: {name} — имя пользователя

Админ: 🎄 **С Новым Годом!**
       
       Дарим -30% на всё до 10 января!
       Промокод: NY2026

Бот:   ✅ Контент для 🇷🇺 RU сохранён.
       
       Теперь отправьте контент для 🇬🇧 EN
       или нажмите [Авто-перевод] / [Пропустить]

Админ: 🎄 **Happy New Year!**
       
       Get -30% off until Jan 10!
       Promo: NY2026

Бот:   ✅ Контент для 🇬🇧 EN сохранён.
       
       🔘 Добавить кнопки? [Да] [Пропустить]
...
Бот:   Отправьте кнопку в формате:
       ...
       Пример: `🛒 Купить | 🛒 Shop | https://t.me/bot?start=promo`

Админ: 🎁 К подаркам | 🎁 Gifts | https://t.me/pvndora_ai_bot?start=promo_ny

Бот:   ✅ Кнопка добавлена. Ещё? [Добавить ещё] [Готово]
...
Бот:   📋 ПРЕВЬЮ РАССЫЛКИ
       ━━━━━━━━━━━━━━━━━━
       🤖 Бот: PVNDORA
       🎯 Аудитория: Активные 7д
       🌐 Языки: RU, EN
       👥 Получателей: ~1,247
       
       [👁 Превью RU] [👁 Превью EN]
       
       [⏰ Запланировать] [🚀 Отправить сейчас]

Админ: [🚀 Отправить сейчас]
...
core/bot/admin/
├── __init__.py
├── middlewares.py      # AdminAuthMiddleware (whitelist telegram_id)
├── handlers/
│   ├── __init__.py
│   ├── broadcast.py    # /broadcast flow
│   ├── stats.py        # /stats, /users
│   └── stock.py        # /stock alerts
└── states.py           # FSM states for broadcast flow
```
...
```env
# Admin Bot
ADMIN_BOT_TOKEN=7123456789:AAHxxxxx
ADMIN_TELEGRAM_IDS=123456789,987654321  # Whitelist админов

# Target bots
TELEGRAM_TOKEN=...           # PVNDORA bot
DISCOUNT_BOT_TOKEN=...       # Discount bot
```

---

## Архитектура (DB-based)
