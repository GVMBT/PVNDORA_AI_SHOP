...
class DiscountOrderService:
    ...
    def __init__(self, db_client):
        ...
        if webapp_url:
            self.base_url = (
                webapp_url.rstrip("/") if webapp_url.startswith("http") else f"https://{webapp_url}"
            )
        elif base_url:
            self.base_url = (
                base_url.rstrip("/") if base_url.startswith("http") else f"https://{base_url}"
            )
        else:
            # Fallback to TELEGRAM_WEBHOOK_URL and extract base
            telegram_webhook = os.environ.get("TELEGRAM_WEBHOOK_URL", "https://pvndora.app")
            self.base_url = (
                telegram_webhook.rsplit("/api", 1)[0].rsplit("/webhook", 1)[0].rstrip("/")
            )
    ...
    async def schedule_delayed_delivery(
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(publish_url, json=payload, headers=headers, timeout=10)
            ...
            logger.exception("Failed to schedule delayed delivery")
            return None

    async def get_estimated_delivery_time(self, order_id: str) -> datetime | None:
        ...
        try:
            ...
            logger.exception("Failed to get delivery time")
            return None
    ...
    async def mark_discount_user(self, telegram_id: int) -> bool:
        ...
        try:
            ...
            logger.exception("Failed to mark discount user")
            return False
    ...
    async def check_terms_accepted(self, telegram_id: int) -> bool:
        ...
        try:
            ...
            logger.exception("Failed to check terms")
            return False
