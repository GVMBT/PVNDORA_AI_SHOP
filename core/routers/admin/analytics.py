...
async def admin_get_analytics(days: int = 7, admin=Depends(verify_admin)):
    ...
    open_tickets_result = (
        await db.client.table("tickets").select("id", count="exact").eq("status", "open").execute()
    )

    revenue_by_day_result = (
        await db.client.table("orders")
        .select("amount, created_at")
        .eq("status", "delivered")
        .gte("created_at", chart_days_start.isoformat())
        .execute()
    )
    ...
    revenue_by_day = [
        {"date": date, "amount": amount} for date, amount in sorted(revenue_by_day_map.items())
    ]
    ...
    top_products = [
        {
            ...
            "sales": count,
            "revenue": 0,
        }  # revenue can be calculated separately if needed
...
async def admin_get_business_metrics(days: int = 30, admin=Depends(verify_admin)):
    ...
    summary = {
        ...
        "avg_daily_revenue": (
            sum(d.get("revenue", 0) for d in daily_data) / len(daily_data) if daily_data else 0
        ),
        "avg_conversion_rate": (
            sum(d.get("order_conversion_rate", 0) for d in daily_data) / len(daily_data)
            if daily_data
            else 0
        ),
        "avg_order_value": (
            sum(d.get("avg_order_value", 0) for d in daily_data)
            / len([d for d in daily_data if d.get("avg_order_value", 0) > 0])
            if any(d.get("avg_order_value", 0) > 0 for d in daily_data)
            else 0
        ),
    }
...
async def admin_get_discount_analytics(days: int = 30, admin=Depends(verify_admin)):
    ...
    insurance_items = (
        await db.client.table("order_items")
        .select("id, insurance_id", count="exact")
        .not_.is_("insurance_id", "null")
        .execute()
    )
    ...
    promo_stats = (
        ...
        .select("source_trigger, current_uses")
