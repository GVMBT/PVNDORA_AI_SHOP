...
from .models import ReferralSettingsRequest, ReviewApplicationRequest, SetPartnerRequest
...
async def admin_update_referral_settings(
    ...
    if request.thresholds_by_currency is not None:
        ...
        if not isinstance(request.thresholds_by_currency, dict):
            raise HTTPException(
                status_code=400, detail="thresholds_by_currency must be a dictionary"
            )
        # Validate each currency has level2 and level3
        for currency, thresholds in request.thresholds_by_currency.items():
            if not isinstance(thresholds, dict):
                raise HTTPException(
                    status_code=400, detail=f"Thresholds for {currency} must be a dictionary"
                )
            if "level2" not in thresholds or "level3" not in thresholds:
                raise HTTPException(
                    status_code=400, detail=f"Thresholds for {currency} must have level2 and level3"
                )
            if thresholds["level2"] < 0 or thresholds["level3"] < 0:
                raise HTTPException(
                    status_code=400, detail=f"Thresholds for {currency} must be >= 0"
                )
...
async def admin_get_referral_metrics(admin=Depends(verify_admin)):
    ...
    return {"overview": stats.data if stats.data else {}, "top_referrers": top_referrers.data or []}
...
async def admin_get_partners_crm(
    ...
    try:
        ...
        result = (
            await db.client.table(view_name)
            .select("*")
        ...
        count_result = await db.client.table(view_name).select("user_id", count="exact").execute()
...
async def admin_get_referral_metrics_detailed(admin=Depends(verify_admin)):
    ...
    pending_bonuses = (
        ...
        .select("amount, level, ineligible_reason")
        .eq("eligible", False)
        .execute()
    )
    ...
    return {
        ...
        "thresholds_usd": {"level1": 50, "level2": 250, "level3": 1000},
    }
...
async def admin_get_partners(admin=Depends(verify_admin)):
    ...
    for p in result.data or []:
        try:
            stats_result = (
                await db.client.table("referral_stats_extended")
                .select("level1_count, level2_count, level3_count, effective_level")
                .eq("user_id", p["id"])
                .execute()
            )
...
async def admin_set_partner(request: SetPartnerRequest, admin=Depends(verify_admin)):
    ...
    error_msg = (
        rpc_result.get("error", "Failed to update partner status")
        if rpc_result
        else "No response from database"
    )
...
async def admin_review_application(request: ReviewApplicationRequest, admin=Depends(verify_admin)):
    ...
    app_result = (
        ...
        .select("id, user_id, status, telegram_id")
    ...
    if request.approve:
        await db.client.rpc(
            "admin_set_partner_level",
            {"p_user_id": user_id, "p_level": request.level_override, "p_is_partner": True},
        ).execute()
