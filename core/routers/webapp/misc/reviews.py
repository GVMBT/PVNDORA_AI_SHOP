...
async def _determine_review_product_id(request: WebAppReviewRequest, order_items: list) -> Optional[str]:
...
def _is_duplicate_key_error(exception: Exception) -> bool:
    ...
    if hasattr(exception, 'code'):
        if exception.code == 409 or str(exception.code) == "23505" or "23505" in str(exception.code):
            return True
    ...
    duplicate_keywords = ["23505", "duplicate key", "unique constraint", "already exists", "conflict"]
