...
from .constants import MIN_TICKET_MESSAGE_LENGTH, MAX_TICKETS_PER_USER
...
async def get_user_tickets(user=Depends(verify_telegram_auth)):
    ...
    ).eq("user_id", db_user.id).order("created_at", desc=True).limit(MAX_TICKETS_PER_USER).execute()
...
async def create_user_ticket(request: CreateTicketRequest, user=Depends(verify_telegram_auth)):
    ...
    if len(request.message.strip()) < MIN_TICKET_MESSAGE_LENGTH:
        raise HTTPException(status_code=400, detail=f"Message too short (min {MIN_TICKET_MESSAGE_LENGTH} characters)")
