...
async def _deliver_items_for_order(
    ...
    try:
        ...
        logger.error(
            f"deliver-goods: failed to load products for order {order_id}: {e}", exc_info=True
        )
    ...
    for it in items:
        ...
        if only_instant and fulfillment_type == "preorder":
            waiting_count += 1
            logger.debug(
                f"deliver-goods: skipping preorder item {it.get('id')} (only_instant=True, will be delivered by auto_alloc)"
            )
            continue
        ...
        try:
            ...
            logger.error(
                f"deliver-goods: stock query failed for order {order_id}, product {product_id}: {e}",
                exc_info=True,
            )
            stock_item = None
        ...
        if stock_item:
            ...
            try:
                ...
                if update_result.data:
                    ...
                    try:
                        ...
                        logger.error(
                            f"deliver-goods: failed to update order_item {item_id}: {e}",
                            exc_info=True,
                        )
                    ...
                    logger.warning(
                        f"deliver-goods: stock {stock_id} was already reserved for order_item {it.get('id')}"
                    )
                    waiting_count += 1
            except Exception as e:
                logger.error(
                    f"deliver-goods: failed to mark stock sold {stock_id}: {e}", exc_info=True
                )
                waiting_count += 1
        else:
            ...
            logger.debug(
                f"deliver-goods: NO stock available for product {product_id}, keeping current status"
            )
            waiting_count += 1
            item_id = it.get("id")
    ...
    logger.debug(
        f"deliver-goods: order {order_id} status calc: total_delivered={total_delivered_final} (prev={total_delivered} + new={delivered_count}), waiting={waiting_count}"
    )
    ...
    try:
        if order_data.data:
            ...
            if not saved_calculated:
                ...
                if saved_amount > 0 and user_id:
                    ...
                    current_saved = (
                        to_float(user_check.data.get("total_saved") or 0) if user_check.data else 0
                    )
        ...
        logger.error(
            f"deliver-goods: Failed to update total_saved for order {order_id}: {e}", exc_info=True
        )
    ...
    try:
        ...
        if delivered_lines:
            ...
            logger.info(
                f"deliver-goods: Order {order_id} is delivered but notification was never sent, fetching content from delivered items"
            )
