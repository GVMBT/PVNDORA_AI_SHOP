...
async def worker_send_broadcast(request: Request):
    ...
    if not broadcast_id or not user_ids:
        logger.error(
            f"Broadcast worker: missing required fields - broadcast_id={broadcast_id}, user_ids_count={len(user_ids) if user_ids else 0}"
        )
    ...
    logger.info(
        f"Broadcast {broadcast_id} found: status={broadcast_result.data.get('status')}, recipients={broadcast_result.data.get('total_recipients')}"
    )
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"Broadcast {broadcast_id}: Downloaded media file, size={len(media_bytes)} bytes"
                        )
                    else:
                        logger.error(
                ...
                logger.exception(
                    f"Broadcast {broadcast_id}: Failed to download media from admin bot"
                )
    ...
    try:
                ...
                for btn in buttons:
                    ...
                    if "url" in btn:
                        ...
                        rows.append(
                            [
                                InlineKeyboardButton(
                                    text=btn_text, callback_data=btn["callback_data"]
                                )
                            ]
                        )
        ...
        logger.info(
            f"Broadcast {broadcast_id}: DB counts - total={total_recipients}, sent={total_sent_in_db}, failed={total_failed_in_db}, processed={total_processed}"
        )
