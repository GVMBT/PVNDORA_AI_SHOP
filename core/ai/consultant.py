...
from core.ai.prompts import get_system_prompt, format_product_catalog_with_currency
from core.ai.tools import TOOLS, execute_tool
...
class AIConsultant:
    ...
    async def get_response(
        ...
        """
        logger.debug(f"AIConsultant.get_response called for user {user_id}, message: {user_message[:50]}...")
        db = get_database()
    ...
    async def get_response_from_voice(
        ...
        try:
            logger.debug(f"Voice message processing - audio size: {len(converted_data)} bytes, mime: {mime_type}")
            ...
            logger.debug(f"Voice response received, candidates: {len(response.candidates) if response.candidates else 0}")
            ...
            error_str = str(e)
            logger.error(f"Gemini voice API error: {error_str}", exc_info=True)
            
            # Check for specific error types
            if "INVALID_ARGUMENT" in error_str or "unsupported" in error_str.lower():
                logger.error("Audio format may not be supported. Telegram sends OGG Opus, Gemini expects OGG Vorbis.")
    ...
    async def _convert_audio_for_gemini(self, ogg_data: bytes) -> tuple[bytes, str]:
        ...
        logger.warning(f"Voice data doesn't have OGG magic bytes, first 4: {ogg_data[:4]!r}")
        return ogg_data, "audio/ogg"
    ...
    async def _process_response(
        ...
        try:
            ...
            if function_calls:
                logger.debug(f"Found {len(function_calls)} function calls: {[fc[0] for fc in function_calls]}")
            ...
            logger.error(f"_process_response failed: {e}", exc_info=True)
            return self._create_error_response(language, str(e))
    ...
    def _parse_structured_response(
        ...
        try:
                            ...
                            if hasattr(candidate.content, 'parts') and candidate.content.parts:
                                for part in candidate.content.parts:
                                    if part and hasattr(part, 'text') and part.text:
                                        text = part.text
                                        logger.debug(f"Extracted text from candidate: {text[:100]}")
                                        break
                                if text:
                                    break
            ...
            logger.error(f"Failed to create structured response: {e}", exc_info=True)
            return self._create_error_response(language, str(e))
    ...
    async def _get_or_create_cache(self, language: str, system_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Context cache created successfully for {language}")
            return cache.name
            ...
            if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                logger.warning(f"Context cache limit exceeded for {language}. Will retry in {RETRY_INTERVAL_HOURS} hour(s).")
                ...
                logger.warning(f"Failed to create context cache for {language}: {e}", exc_info=True)
    ...
    async def _generate_structured_response(
        ...
        try:
                        ...
                        if hasattr(candidate.content, 'parts') and candidate.content.parts:
                            for part in candidate.content.parts:
                                if part and hasattr(part, 'text') and part.text:
                                    ...
                                    logger.debug(f"Extracted initial text from candidate: {initial_text[:100]}")
                                    break
                            if initial_text:
                                break
            ...
            if not initial_text:
                logger.error("No text available for structured response generation")
                return StructuredAIResponse(
            ...
            logger.error(f"_generate_structured_response failed: {e}", exc_info=True)
    ...
    async def _continue_with_all_tool_results(
        ...
        try:
            ...
            logger.error(f"Gemini multi-tool follow-up error: {e}", exc_info=True)
            return self._create_error_response(language, str(e))
