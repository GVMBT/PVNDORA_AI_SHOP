...
const ProfileConnected: React.FC<ProfileConnectedProps> = ({
    ...
    try {
      ...
      if (!preview.can_withdraw || preview.max_amount <= 0) {
        ...
        const minAmountFormatted = formatPrice(
          preview.min_amount,
          (preview.amount_requested_currency as CurrencyCode | undefined) || currency
        );
        showModalAlert(
          t("profile.errors.insufficientFunds"),
      ...
      showWithdraw({
        ...
        onConfirm: async (amount: number, method: string, details: string) => {
          ...
          hapticFeedback?.("notification", "success");
          showModalAlert(
            t("profile.withdraw.submittedTitle"),
            t("profile.withdraw.submittedDesc"),
            "success"
          );
          await getProfile();
        },
  ...
  const handleUpdatePreferences = useCallback(
    async (preferred_currency?: string, interface_language?: string) => {
      logger.info("handleUpdatePreferences called", { preferred_currency, interface_language });

      applyOptimisticUpdates(preferred_currency, interface_language);
      ...
      try {
        ...
        if (preferred_currency) {
          await reloadProfileForCurrency(preferred_currency);
        }

        return result;
      } catch (error) {
        logger.error("Error updating preferences", error);
        rollbackPreferences(preferred_currency, interface_language);
        throw error;
      }
    },
  ...
  const handleSetPartnerMode = useCallback(
    async (mode: "commission" | "discount") => {
      try {
  ...
  const handleSubmitPartnerApplication = useCallback(
    async (data: PartnerApplicationData) => {
      try {
