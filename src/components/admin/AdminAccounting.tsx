           ...
           <div className="space-y-4 relative z-10">
             ...
             <div className="flex justify-between items-end border-b border-white/5 pb-2">
               ...
               <div className="text-right">
                 <div className="text-xl font-mono text-white font-bold">{formatMoney(rubData.revenue_gross || 0, 'RUB')}</div>
             ...
             {/* Скидки в рублях */}
             <div className="flex justify-between items-end border-b border-white/5 pb-2">
               <span className="text-gray-400 text-sm flex items-center gap-1"><Percent size={12}/> Скидки (промокоды)</span>
               <div className="text-right">
                 <div className="text-lg font-mono text-red-400">-{formatMoney(rubData.discounts_given || 0, 'RUB')}</div>
               </div>
             </div>
             ...
             <div className="flex justify-between items-end pt-2">
               ...
               <div className="text-right">
                 <div className="text-2xl font-mono text-blue-400 font-bold">{formatMoney(rubData.revenue || 0, 'RUB')}</div>
        ...
        <div className="bg-[#0e0e0e] border border-white/10 p-5 rounded-sm relative overflow-hidden">
           ...
           <div className="space-y-4 relative z-10">
             ...
             <div className="flex justify-between items-end border-b border-white/5 pb-2">
               <span className="text-gray-400 text-sm">Валовая выручка</span>
               <div className="text-right">
                 <div className="text-xl font-mono text-white font-bold">{formatMoney(usdData.revenue_gross || 0, 'USD')}</div>
                 <div className="text-xs text-gray-500">Цена товаров до скидок</div>
               </div>
             </div>
             ...
             <div className="flex justify-between items-end border-b border-white/5 pb-2">
               <span className="text-gray-400 text-sm flex items-center gap-1"><Percent size={12}/> Скидки (промокоды)</span>
               <div className="text-right">
                 <div className="text-lg font-mono text-red-400">-{formatMoney(usdData.discounts_given || 0, 'USD')}</div>
               </div>
             </div>
             ...
             <div className="flex justify-between items-end pt-2">
               ...
               <div className="text-right">
                 <div className="text-2xl font-mono text-green-400 font-bold">{formatMoney(usdData.revenue || 0, 'USD')}</div>
      ...
      <div className="bg-[#0e0e0e] border border-white/10 p-6 rounded-sm">
        ...
        <div className="mb-6 bg-[#151515] p-4 rounded-sm">
             <div className="flex justify-between items-center mb-2">
               <span className="text-gray-400 text-sm uppercase">1. Общая Чистая Выручка (Total Net Revenue)</span>
               <span className="text-white font-mono font-bold text-xl">{formatMoney(d.totalRevenue, 'USD')}</span>
             </div>
             <div className="text-xs text-gray-600 text-right">
               * Сумма рублевой и долларовой выручки (рубли конвертированы в USD по курсу на момент оплаты)
             </div>
             
             {d.totalInsuranceRevenue > 0 && (
               <div className="flex justify-between items-center mt-2 pt-2 border-t border-white/5 text-sm">
                 <span className="text-gray-400 pl-4 flex items-center gap-2"><Shield size={12}/> + Доход от страховок</span>
                 <span className="text-green-400 font-mono">{formatMoney(d.totalInsuranceRevenue, 'USD')}</span>
               </div>
             )}
        </div>
        ...
        {/* 2. Расходы */}
        <div className="mb-6">
          <div className="text-xs uppercase text-red-400 font-mono mb-3 px-2">2. Глобальные расходы (USD)</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 px-4">
            ...
            <div className="col-span-1 md:col-span-2 flex justify-between items-center border-b border-white/5 pb-2 mb-2">
               ...
               <span className="text-red-400 font-mono font-bold">{formatMoney(d.totalCogs, 'USD')}</span>
            </div>

            <MetricRow label="Эквайринг (комиссии)" value={d.totalAcquiringFees} isExpense displayCurrency="USD" />
            <MetricRow label="Реферальные выплаты" value={d.totalReferralPayouts} isExpense displayCurrency="USD" />
            <MetricRow label="Резервы (маркетинг 5%)" value={d.totalReserves} isExpense displayCurrency="USD" />
            <MetricRow label="Кэшбэк за отзывы" value={d.totalReviewCashbacks} isExpense displayCurrency="USD" />
            <MetricRow label="Замены по гарантии" value={d.totalReplacementCosts} isExpense displayCurrency="USD" />
            {d.totalOtherExpenses > 0 && (
              <MetricRow label="Прочие расходы" value={d.totalOtherExpenses} isExpense displayCurrency="USD" />
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-pandora-cyan/5 to-transparent border-l-4 border-pandora-cyan p-6 mt-8">
           <div className="flex justify-between items-start">
             <div>
               <h3 className="text-2xl font-bold text-white uppercase font-display mb-1">Чистая Прибыль</h3>
               <p className="text-xs text-gray-500 max-w-[300px]">
                 (Рублевая выручка + Долларовая выручка) - Все расходы = Net Profit
               </p>
             </div>
             <div className="text-right">
               <div className={`text-4xl font-mono font-bold ${d.netProfit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="bg-[#0e0e0e] border border-yellow-500/30 p-6 rounded-sm">
            ...
            <div className="flex justify-between items-end">
              <div>
                ...
                <div className={`text-2xl font-mono ${reservesAvailable >= 0 ? 'text-white' : 'text-red-400'}`}>
                  {formatMoney(reservesAvailable, 'USD')}
                </div>
              </div>
              <div className="text-right">
                 <div className="text-xs text-gray-500">Всего отложено: {formatMoney(reservesAccumulated, 'USD')}</div>
                 <div className="text-xs text-gray-500">Потрачено: {formatMoney(reservesUsed, 'USD')}</div>
              </div>
            </div>
          </div>
                ...
                {Object.entries(liabilitiesByCurrency).map(([curr, val]) => (
                   <div key={curr} className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-mono">
                        {formatCurrencyAmount((val.user_balances || 0) + (val.pending_withdrawals || 0), curr)}
                      </span>
                   </div>
                ))}
             </div>
