...
const AdminSales: React.FC<AdminSalesProps> = ({ orders, onRefresh }) => {
  ...
  const handleForceStatus = useCallback(async (orderId: string, newStatus: string) => {
    ...
    <div className="space-y-6">
        ...
        <table className="w-full text-left text-xs font-mono">
          ...
          <tbody className="divide-y divide-white/5 text-gray-300">
            {filteredOrders.map(o => {
              ...
              return (
                <tr key={o.id} className={`hover:bg-white/5 transition-colors ${isPending ? 'bg-yellow-500/5' : ''}`}>
                  ...
                  <td className="p-4 text-right">
                    {isPending && (
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleCheckPayment(o.id); }}
                        ...
                        >
                          <RefreshCw size={12} className={`text-blue-400 ${isChecking ? 'animate-spin' : ''}`} />
                        </button>
                        <button
                          onClick={(e) => { e.stopPropagation(); handleForceStatus(o.id, 'paid'); }}
                          disabled={isChecking || isProcessing}
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleForceStatus(o.id, 'cancelled'); }}
                          disabled={isChecking || isProcessing}
      ...
      <div className="md:hidden space-y-4">
        {filteredOrders.map(o => {
          ...
          return (
            <div key={o.id} className={`bg-[#0e0e0e] border ${isPending ? 'border-yellow-500/30' : 'border-white/10'} p-4 space-y-3`}>
