...
const Profile: React.FC<ProfileProps> = ({ onBack, onHaptic, onAdminEnter }) => {
  ...
  const displayedNodes = NETWORK_TREE.filter(n => n.line === networkLine);

  // Logic for Career Progress
  const currentTurnover = MOCK_USER.stats.turnover;
  const currentLevel = CAREER_LEVELS.find(l => currentTurnover >= l.min && currentTurnover < l.max) || CAREER_LEVELS[CAREER_LEVELS.length - 1];
  const nextLevel = CAREER_LEVELS.find(l => l.id === currentLevel.id + 1);
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(MOCK_USER.referralLink);
    setCopied(true);
  ...
  const toggleRewardMode = () => {
      if (!MOCK_USER.isVip && !MOCK_USER.role.includes('ADMIN')) return;
      if (onHaptic) onHaptic('medium');
            ...
            <div className="flex items-center gap-6">
                ...
                <div>
                    <h2 className="text-2xl font-display font-bold text-white tracking-tight mb-1 flex items-center gap-2">
                        {MOCK_USER.name}
                        {MOCK_USER.isVip && <Crown size={18} className="text-yellow-500 fill-yellow-500/20" />}
                    </h2>
                    <div className="flex items-center gap-3 text-xs font-mono text-gray-500">
                        <span>{MOCK_USER.handle}</span>
                        <span className="text-pandora-cyan">//</span>
                        <span>{MOCK_USER.id}</span>
                ...
                <div className="bg-[#080808] border border-white/10 p-6 relative overflow-hidden h-full flex flex-col justify-between group hover:border-pandora-cyan/30 transition-all">
                    ...
                    <div className="mb-6">
                        ...
                        <div className="text-4xl sm:text-5xl font-display font-bold text-white flex items-baseline gap-2">
                            <DecryptedText text={MOCK_USER.balance} /> <span className="text-xl text-pandora-cyan">₽</span>
                        </div>
                    </div>
                     ...
                     <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                        ...
                        <div className="flex items-center gap-6 text-[10px] font-mono bg-white/5 px-4 py-2 rounded-sm border border-white/5 w-full sm:w-auto justify-between sm:justify-start">
                            <div className="flex flex-col items-center sm:items-start">
                                ...
                                <span className="text-white font-bold text-base">{MOCK_USER.stats.clicks}</span>
                            </div>
                            <div className="w-px h-6 bg-white/10" />
                            <div className="flex flex-col items-center sm:items-start">
                                <span className="text-gray-500">CONVERSION</span>
                                <span className="text-pandora-cyan font-bold text-base">{MOCK_USER.stats.conversion}%</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex-1 bg-black border border-white/20 p-4 relative overflow-hidden flex flex-col justify-center min-h-[100px]">
                            ...
                            <div className="relative z-10">
                                ...
                                <code className="text-lg font-mono text-white font-bold tracking-widest break-all">
                                    {MOCK_USER.referralLink.split('/').pop()}
                                </code>
                            </div>
                        </div>
