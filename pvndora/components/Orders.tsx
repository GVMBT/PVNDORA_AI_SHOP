...
const Orders: React.FC<OrdersProps> = ({ onBack, onOpenSupport }) => {
  ...
  const [reviewModal, setReviewModal] = useState<{isOpen: boolean, itemId: number | null, itemName: string}>({
  ...
  const openReviewModal = (itemId: number, itemName: string) => {
      setRating(5);
      setReviewText('');
      setReviewModal({ isOpen: true, itemId, itemName });
  };

  const submitReview = () => {
      if (!reviewModal.itemId) return;

      // Update mock state to show "Reviewed" status
      ...
      setReviewModal({ isOpen: false, itemId: null, itemName: '' });
  };

  const filteredOrders = ordersState.filter(order => {
    if (activeTab === 'all') return true;
    if (activeTab === 'active') return order.status === 'processing' || order.status === 'paid'; 
    if (activeTab === 'log') return order.status === 'refunded'; 
    return true;
  });
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    ...
                    >
                        ...
                        <button 
                            ...
                            className="w-full bg-white/5 hover:bg-pandora-cyan border border-white/20 hover:border-pandora-cyan text-white hover:text-black font-bold py-3 uppercase tracking-widest transition-all flex items-center justify-center gap-2 group/btn"
                        >
                            <Send size={16} />
                            UPLOAD_PACKET
                        </button>
