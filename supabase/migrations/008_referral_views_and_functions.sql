...
-- ============================================================
-- Detailed analytics for each partner
...
SELECT 
    ...
    COUNT(DISTINCT r.id) AS users_attracted,
    COUNT(DISTINCT CASE WHEN o.status = 'delivered' THEN r.id END) AS buyers_attracted,
    COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.amount ELSE 0 END), 0) AS total_structure_turnover,
    COALESCE(SUM(rb.amount), 0) AS earned_by_partner,
    COALESCE(u.turnover_usd, 0) AS personal_turnover_usd,
    COALESCE(u.total_referral_earnings, 0) AS total_referral_earnings,
    COALESCE(u.referral_program_unlocked, false) AS referral_program_unlocked,
...
GROUP BY u.id, u.telegram_id, u.username, u.first_name, u.is_partner, u.partner_level_override, 
         u.turnover_usd, u.total_referral_earnings, u.referral_program_unlocked, u.created_at;
