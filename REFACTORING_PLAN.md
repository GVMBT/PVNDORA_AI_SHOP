...
**Дата:** 2026-01-27  
**Последнее обновление:** 2026-01-27 (расширенное исследование противоречий)  
...
**Проблема:** `CurrencyService.convert_price()` работает только из USD, а здесь нужна конвертация между любыми валютами (RUB→EUR, USD→RUB, etc.).

**Решение:** 
1. Создать метод `CurrencyService.convert_balance(from_currency: str, to_currency: str, amount: float) -> float`
...
**Следующие шаги:**
1. Phase 0: Консолидация дублирований (Telegram messaging + Currency conversion)
2. Phase 7: Database Query Optimization (N+1 queries) — критично для масштабирования
3. Phase 2: Split Notifications & Workers — после Phase 0
